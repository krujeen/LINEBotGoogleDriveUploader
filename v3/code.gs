...
// 🔒 รหัสผ่านสำหรับการยืนยัน (plain-text หรือใช้ hash ก็ได้)
const ADMIN_PASSWORD = 'yourpassword';

// ใช้สำหรับยืนยันรหัสผ่าน
 ...
 * เปลี่ยนชื่อไฟล์
 */
function renameFile(fileId, newName, password) {
  try {
    if (!verifyPassword(password)) {
      throw new Error("รหัสผ่านไม่ถูกต้อง");
    }
 ...
 * เปลี่ยนชื่อโฟลเดอร์
 */
function renameFolder(folderId, newName, password)  {
  try {
    if (!verifyPassword(password)) {
      throw new Error("รหัสผ่านไม่ถูกต้อง");
    }
 ...
 * ลบโฟลเดอร์
 */
function deleteFolder(folderId, password) {
  try {
    if (!verifyPassword(password)) {
      throw new Error("รหัสผ่านไม่ถูกต้อง");
    }
